// Write places-3d/manifest.json - the /places-3d visualizer pages that use a
...
//
// It scans every OUT/<slug>/descriptor.json written by build-places-3d.sh,
// hashes each file the descriptor lists, and emits one manifest for the whole
// places-3d asset set. Add a new asset by teaching build-places-3d.sh to emit its
...
if (slugDirs.length === 0) {
  console.error("[make-places-3d-manifest] FATAL: no <slug>/descriptor.json found under " + OUT);
  process.exit(1);
}
...
for (const slug of slugDirs) {
  ...
  desc.files = desc.files.map((f) => {
    ...
    if (!fs.existsSync(abs)) {
      console.error(`[make-places-3d-manifest] FATAL: ${slug} lists missing file ${f.file}`);
      process.exit(1);
    }
...
const manifest = {
  schema: "ftol-places-3d/1",
  version: VERSION,
  built_iso: new Date().toISOString(),
  note: "Real-terrain datasets (Terrarium DEM + Sentinel-2 texture) for freetoolonline.com /places-3d detailed-render pages. Pages not yet migrated render fully procedurally and use nothing here.",
  assets,
};

fs.writeFileSync(path.join(OUT, "manifest.json"), JSON.stringify(manifest, null, 2) + "\n");
console.log(`[make-places-3d-manifest] wrote manifest.json (version ${VERSION}, ${slugDirs.length} asset(s): ${slugDirs.join(", ")})`);
