...
Egg Time Rewind
FTOL adapt 2026-07-20: stripped debug/CCapture; localStorage -> ftol:eggtimerewind:highScore
By Frank Force 2020
...
{  
    ...
    if (gameMode == 1)
    {
        if (localStorage["ftol:eggtimerewind:highScore"])
            DrawText('High:'+localStorage["ftol:eggtimerewind:highScore"],42,Min(-6+levelTimer*4,1)|0,1,2);
...
function AddToScore(score)
{
    ...
    playerScore += score;
    if (!localStorage["ftol:eggtimerewind:highScore"] || playerScore > localStorage["ftol:eggtimerewind:highScore"])
    {
        localStorage["ftol:eggtimerewind:highScore"] = playerScore;
        newHighScore = 1;
    }
}
