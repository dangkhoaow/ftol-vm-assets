...
function Reset()
{
    ...
    if (localStorage['ftol:bounceback:coins'])
        playerData.coins = parseInt(localStorage['ftol:bounceback:coins'], 10);
    if (localStorage['ftol:bounceback:warp'])
        warpLevel = parseInt(localStorage['ftol:bounceback:warp'], 10);
    if (localStorage['ftol:bounceback:bestTime'])
        speedRunBestTime = parseInt(localStorage['ftol:bounceback:bestTime'], 10);
...
function NextLevel()
{
    ...
    localStorage['ftol:bounceback:warp'] = warpLevel;
...
function Update()
{
    ...
    // save data
    if (!speedRunMode)
        localStorage['ftol:bounceback:coins'] = playerData.coins;
    ...
    Kill()
    {
        ...
        if (this.type)
        {
            ...
            if (isFinalLevel)
            {
                ...
                winTimer.Set();
                localStorage['ftol:bounceback:warp']=0;
                localStorage['ftol:bounceback:won']=1;
                ...
                if (speedRunMode && (!speedRunBestTime || speedRunTime < speedRunBestTime))
                {
                    ...
                    localStorage['ftol:bounceback:bestTime']=speedRunBestTime;
                }
                PlaySound(2);
            }
        }
...
function GenerateLevelInternal()
{
    ...
    if (isStartLevel)
    {
        ...
        if (localStorage['ftol:bounceback:won'])
