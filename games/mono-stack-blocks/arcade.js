(function () {
  ...
  // persistence with namespaced localStorage (ftol:monostackblocks:storage).
  function acquireVsCodeApi() {
    const KEY = 'ftol:monostackblocks:storage';
    function load() {
      try {
  ...
  const GAME_META = {
    ...
    blocks: {
      id: 'blocks',
      name: 'Stack Blocks',
      tagline: 'Stack tetrominoes, clear lines, chase your best score.',
      hsKey: 'blocks',
    },
    sweeper: {
      id: 'sweeper',
  ...
  // Static boot: hydrate from localStorage, then open stack blocks.
  (function boot() {
    try {
      const raw = localStorage.getItem('ftol:monostackblocks:storage');
      if (raw) {
        const parsed = JSON.parse(raw);
        state.storage = {
          highScores: parsed.highScores || {},
