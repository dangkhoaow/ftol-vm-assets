...
let score = 0;
let highScore = localStorage.getItem('ftol:neondeepspace:high_score') ? parseInt(localStorage.getItem('ftol:neondeepspace:high_score')) : 0;
let currentEnergy = localStorage.getItem('ftol:neondeepspace:energy') ? parseInt(localStorage.getItem('ftol:neondeepspace:energy')) : 0;
let playerUpgrades = {};
...
try {
    const savedUpgrades = localStorage.getItem('ftol:neondeepspace:upgrades');
    if (savedUpgrades) {
...
function resetSave() {
    if (confirm("确定要重置存档吗？\n这将清除所有能量、升级和历史最高分！")) {
        localStorage.removeItem('ftol:neondeepspace:energy');
        localStorage.removeItem('ftol:neondeepspace:upgrades');
        localStorage.removeItem('ftol:neondeepspace:high_score');
        location.reload();
    }
}
