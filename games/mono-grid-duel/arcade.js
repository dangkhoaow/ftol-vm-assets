(function () {
  ...
  // persistence with namespaced localStorage (ftol:monogridduel:storage).
  function acquireVsCodeApi() {
    const KEY = 'ftol:monogridduel:storage';
    function load() {
      try {
  ...
  // Static boot: hydrate from localStorage, then open grid duel.
  (function boot() {
    try {
      const raw = localStorage.getItem('ftol:monogridduel:storage');
      if (raw) {
        const parsed = JSON.parse(raw);
        state.storage = {
          highScores: parsed.highScores || {},
