(function () {
  ...
  // persistence with namespaced localStorage (ftol:headsoccerarena:storage).
  function acquireVsCodeApi() {
    const KEY = 'ftol:headsoccerarena:storage';
    function load() {
      try {
  ...
  (function boot() {
    try {
      const raw = localStorage.getItem('ftol:headsoccerarena:storage');
      if (raw) {
        const parsed = JSON.parse(raw);
        state.storage = {
          highScores: parsed.highScores || {},
