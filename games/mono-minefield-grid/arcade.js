(function () {
  ...
  // persistence with namespaced localStorage (ftol:monominefieldgrid:storage).
  function acquireVsCodeApi() {
    const KEY = 'ftol:monominefieldgrid:storage';
    function load() {
      try {
  ...
  // Static boot: hydrate from localStorage, then open the minefield.
  (function boot() {
    try {
      const raw = localStorage.getItem('ftol:monominefieldgrid:storage');
      if (raw) {
        const parsed = JSON.parse(raw);
        state.storage = {
          highScores: parsed.highScores || {},
