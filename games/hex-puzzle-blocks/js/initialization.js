...
function initialize(a) {
	...
	window.op = 0;
	window.saveState = localStorage.getItem("ftol:hexpuzzleblocks:saveState") || "{}";
	if (saveState !== "{}") {
		op = 1;
	}
	...
	highscores = [];
	if (localStorage.getItem('ftol:hexpuzzleblocks:highscores')) {
		try {
			highscores = JSON.parse(localStorage.getItem('ftol:hexpuzzleblocks:highscores'));
		} catch (e) {
			highscores = [];
		}
	}
	...
	if (a != 1) {
		...
		$(window).unload(function() {
			...
			if (gameState == 1 || gameState == -1 || gameState === 0) localStorage.setItem("ftol:hexpuzzleblocks:saveState", exportSaveState());
			else localStorage.setItem("ftol:hexpuzzleblocks:saveState", "{}");
		});
