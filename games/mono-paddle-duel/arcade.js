(function () {
  ...
  // persistence with namespaced localStorage (ftol:monopaddleduel:storage).
  function acquireVsCodeApi() {
    const KEY = 'ftol:monopaddleduel:storage';
    function load() {
      try {
  ...
  // Static boot: hydrate from localStorage, then open paddle duel.
  (function boot() {
    try {
      const raw = localStorage.getItem('ftol:monopaddleduel:storage');
      if (raw) {
        const parsed = JSON.parse(raw);
        state.storage = {
          highScores: parsed.highScores || {},
