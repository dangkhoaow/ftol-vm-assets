...
function showMainMenu() {
    ...
    const hasSave = localStorage.getItem("ftol:serversurvivaltd:serverSurvivalSave") !== null;
    if (loadBtn) {
...
window.saveGameState = (saveAs = "browser") => {
    try {
        ...
        if(saveAs === "file")
            ...
            localStorage.setItem("ftol:serversurvivaltd:serverSurvivalSave", JSON.stringify(saveData));
...
function loadGameState(saveData = null) {
    try {
        ...
        if(!saveData){
            const saveDataStr = localStorage.getItem("ftol:serversurvivaltd:serverSurvivalSave");
            if (!saveDataStr) {
